...
#define MAX_THRESHOLD 10
  /* enought work for every thieves */
  if (range_size >= MAX_THRESHOLD * nreq)
  {
    unit_size = MAX_THRESHOLD;
  } else { /* or not enought */
    unit_size = range_size / (nreq + 1);
...
split:
  PC2X ("WQ %d have range_size = %d so unit_size = %d for %d thieves\n",
      priority, range_size, unit_size, nreq);
  ...
  GET_TICK(tp1);
#endif
  ret = c2x_workqueue_steal(wq, &i, &j, unit_size * nreq);
#ifdef C2X_USES_TIMING
  GET_TICK(tp2);
  wq_time_table[c2x_tid].tpop += TICK_RAW_DIFF(tp1,tp2);
#endif
  ...
  for ( /* void */; 
      ((nreq != 0) && !kaapi_listrequest_iterator_empty(lri));
      --nreq, ++nrep, i = (i + unit_size) % work.wq_2.size
      )  
  {
    kaapi_request_t* req = kaapi_listrequest_iterator_get(lr, lri);
    PC2X ("Replying [%d;%d]\n", i, i + unit_size - 1);
